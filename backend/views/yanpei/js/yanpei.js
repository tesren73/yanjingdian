var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		...
		initDom: function(a) {
			var b = this;
			if (this.$_customer = $("#customer"), this.$_date = $("#date").val(system.endDate), this.$_number = $("#number"), this.$_note = $("#note"), this.$_discountRate = $("#discountRate"), this.$_deduction = $("#deduction"), this.$_discount = $("#discount"), this.$_payment = $("#payment"), this.$_arrears = $("#arrears"), this.$_totalArrears = $("#totalArrears"), this.$_toolTop = $("#toolTop"), this.$_toolBottom = $("#toolBottom"), this.$_paymentTxt = $("#paymentTxt"), this.$_accountInfo = $("#accountInfo"), this.$_userName = $("#userName"), this.$_modifyTime = $("#modifyTime"), this.$_createTime = $("#createTime"), this.$_checkName = $("#checkName"), this.$_customerFree = $("#customerFree"), this.$_zhouxiang = $("#zhouxiang"), this.$_optName = $("#optName"), this.$_optLinkName = $("#optLinkName"), this.$_rightsph = $("#rightsph"), this.$_rightcyl = $("#rightcyl"), this.$_rightax = $("#rightax"), this.$_pd = $("#pd"), this.$_Rlengjing = $("#Rlengjing"), this.$_Rjidi = $("#Rjidi"), this.$_Rcorrected = $("#Rcorrected"), this.$_Rluoshi = $("#Rluoshi"), this.$_Radd = $("#Radd"), this.$_Rtonggao = $("#Rtonggao"), this.$_Rjihu = $("#Rjihu"), this.$_RPD = $("#RPD"), this.$_opttype1 = $("#opttype1"), this.$_leftsph = $("#leftsph"), this.$_leftcyl = $("#leftcyl"), this.$_leftax = $("#leftax"), this.$_Llengjing = $("#Llengjing"), this.$_Ljidi = $("#Ljidi"), this.$_Lcorrected = $("#Lcorrected"), this.$_Lluoshi = $("#Lluoshi"), this.$_Ladd = $("#Ladd"), this.$_Ltonggao = $("#Ltonggao"), this.$_Ljihu = $("#Ljihu"), this.$_LPD = $("#LPD"), this.$_ecsdate = $("#ecsdate"), this.$_ygdate = $("#ygdate").val(system.endDate), this.$_remark = $("#remark"), this.customerArrears = 0, this.customerArrears = 0, this.$_note.placeholder(), "150602" == originalData.transType) {
				parent.$("#page-tab").find("li.l-selected").children("a").html("配镜退货单");
				$("#paymentTxt").html("本次退款")
			} else {
				parent.$("#page-tab").find("li.l-selected").children("a").html("验配单");
				$("#paymentTxt").html("本次收款")
			}
			this.yanguangCombo = Business.billSalesCombo($("#yanguangshi"), {
				defaultSelected: c
			});
			this.dgCombo = Business.billSalesCombo($("#dgsales"), {
				defaultSelected: c
			});
			...
			if (this.salesCombo = Business.billSalesCombo($("#sales"), {
				...
				for (var e = 0; e < SYSTEM.salesInfo.length; e++) if (SYSTEM.salesInfo[e].id === a.salesId) {
					...
					var c = {
						id: a.cid ? a.cid : a.oid,
						name: a.name,
						cLevel: a.cLevel
					};
					b.$_customer.data("contactInfo", c), b.customerCombo.input.val(a.oid + ' ' + a.name), b.setSaleByContact(c)
		...
		loadGrid: function(a) {
			...
			w.mod_PageConfig.gridReg(C, B), B = w.mod_PageConfig.conf.grids[C].colModel, $("#grid").jqGrid({
				...
				afterSaveCell: function(a, c, d, e, f) {
					...
					case "qty":
						var d = parseFloat(d),
							g = parseFloat($("#grid").jqGrid("getCell", a, f + 4)),
							h = parseFloat($("#grid").jqGrid("getCell", a, f + 5));
					...
					case "discountRate":
						var d = parseFloat(d),
							l = parseFloat($("#grid").jqGrid("getCell", a, f - 5)),
					...
					case "deduction":
						var d = parseFloat(d),
							l = parseFloat($("#grid").jqGrid("getCell", a, f - 6)),
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && c.ajaxPost("../scm/invSa/addNew?action=addNew", {
					...
					if (200 === b.status) {
						...
						for (var c = 1; 5 >= c; c++) $("#grid").jqGrid("addRowData", c, {});
				...
				if (a.preventDefault(), Business.verifyRight("SA_ADD")) {
					var b = "验配单",
						c = "yanpei-add";
					if ("150602" == originalData.transType) var b = "配镜退货单",
						c = "sales-salesBack";
					parent.tab.overrideSelectedTabItem({
						tabid: c,
						text: b,
						url: "../settings/yanpei_add?action=initSale&transType=" + originalData.transType
					})
				}
			...
			if (f.length > 0) {
				var g = $.trim(b.$_note.val()),
					h = {
						...
						number: e.number,
						name: e.name,
						optname: $.trim(b.$_optName.val()),
						rightsph: $.trim(b.$_rightsph.val()),
						rightcyl: $.trim(b.$_rightcyl.val()),
						rightax: $.trim(b.$_rightax.val()),
						pd: $.trim(b.$_pd.val()),
						Rlengjing: $.trim(b.$_Rlengjing.val()),
						Rjidi: $.trim(b.$_Rjidi.val()),
						Rcorrected: $.trim(b.$_Rcorrected.val()),
						Rluoshi: $.trim(b.$_Rluoshi.val()),
						Radd: $.trim(b.$_Radd.val()),
						Rtonggao: $.trim(b.$_Rtonggao.val()),
						Rjihu: $.trim(b.$_Rjihu.val()),
						RPD: $.trim(b.$_RPD.val()),
						opttype: b.$_opttype1.checked ? 1 : 0,
						leftsph: $.trim(b.$_leftsph.val()),
						leftcyl: $.trim(b.$_leftcyl.val()),
						leftax: $.trim(b.$_leftax.val()),
						Llengjing: $.trim(b.$_Llengjing.val()),
						Ljidi: $.trim(b.$_Ljidi.val()),
						Lcorrected: $.trim(b.$_Lcorrected.val()),
						Lluoshi: $.trim(b.$_Lluoshi.val()),
						Ladd: $.trim(b.$_Ladd.val()),
						Ltonggao: $.trim(b.$_Ltonggao.val()),
						Ljihu: $.trim(b.$_Ljihu.val()),
						LPD: $.trim(b.$_LPD.val()),
						yanguangshi: b.yanguangCombo.getText(),
						yanguangshiId: b.yanguangCombo.getValue(),
						dgsales: b.dgCombo.getText(),
						dgsalesId: b.dgCombo.getValue(),
						ygdate: $.trim(b.$_ygdate.val()),
						remark: $.trim(b.$_remark.val()),
						ecsdate: $.trim(b.$_ecsdate.val())
					};
