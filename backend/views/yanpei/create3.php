...
<table  class="table11_3">
  ...
  <tr>
    ...
    <td colspan="5">
    <?= $form->field($modelInf, 'name')->widget(MultipleInput::className(), [
						'max' => 10,
						'cloneButton' => true,
						'columns' => [
							[
...
$js = <<<JS
    ...
    //var number = document.getElementById("invoiceinfo-name-0-product_id");//获得指定id值的对象
    //var index = number.selectedIndex;
    //var selectedValue = number.options[index].text;
...
//               }
        ...
        //var number = $("#invoiceinfo-name-"+key[2]+"-number" option:selected").text();//监听select改变，获取下拉选项值 
		...
		$.ajax({
			...
			success : function(data){
                ...
                document.getElementById("invoiceinfo-name-"+sid[2]+"-degrees").value = data[0]['degrees'];
            	//$("#invoiceinfo-name-"+sid[2]+"-degrees").val(obj[0]['degrees']);//解析后的数据相应值放到对应ID的文本框中
            	//$("#invoiceinfo-name-"+sid[2]+"-degrees").val(data[0]['degrees']);
                document.getElementById("invoiceinfo-name-"+sid[2]+"-astigmia").value = data[0]['astigmia'];
                document.getElementById("invoiceinfo-name-"+sid[2]+"-saleprice").value = data[0]['saleprice'];
